...
$mbatt_store_id           = $store_user->get_id();
$mbatt_store_name		  = get_userdata($mbatt_store_id)->data->user_nicename;
$mbatt_first_name		  = get_userdata($mbatt_store_id)->data->first_name;
...
<div class="dokan-profile-frame-wrapper">
                    ...
                    <div class="profile-info">
                        ...
                        <ul class="dokan-store-info">
                            ...
                            <?php endif ?>
                                ...
                                <li class="dokan-store-state">
                                <?php 
                                    $address = dokan_get_seller_address($mbatt_store_id,true);
                                    echo $address['state'] ;
                                    echo ", ";
                                    echo $address['country'] ;
                                    ?>
                                </li>
                                <li class="dokan-store-primary-decipline">
                                     <?php echo  get_user_meta( $store_user->get_id(), 'primary_discipline', true );?>
                                </li>
                        ...
                        <?php if($vendor_statement = get_user_meta($store_user->get_id() , 'vendor_statement', true )){?>
                            <div class="extra_info">
                                <?php $read_more = '...<a href="/store/' . $mbatt_store_name .'/biography">Read More</a>'; ?>
                                <p class="text-center"><?php echo wp_trim_words( $vendor_statement , 45, $read_more ); ?></p>
                            </div>
                        <?php } ?>
    ...
    <div class="dokan-store-tabs<?php echo esc_attr( $no_banner_class_tabs ); ?>">
            <ul class="dokan-list-inline">
                ...
                <li><a href="/store/<?php echo $mbatt_store_name ?>/biography">About <?php echo esc_html( $store_user->get_shop_name() ); ?></a></li><!-- /store/vendor_username-->
            </ul>
        </div>
</div>
...
//mbatt-hide dokan bio
	var mbattXpath = "//a[text()='Reviews']";
	var mbattMatchingElement = document.evaluate(mbattXpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	var mbattParent = mbattMatchingElement.parentElement;
	console.log(mbattParent);
	console.log(mbattMatchingElement);
	mbattParent.classList.add('mbatt-hidden');
//end mbatt-hide dokan bio

//mbatt-hide dokan bio
	var mbattXpath2 = "//a[text()='Artist Biography']";
	var mbattMatchingElement2 = document.evaluate(mbattXpath2, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	var mbattParent2 = mbattMatchingElement2.parentElement;
	console.log(mbattParent2);
	console.log(mbattMatchingElement2);
	mbattParent2.classList.add('mbatt-hidden');
//end mbatt-hide dokan bio
</script>